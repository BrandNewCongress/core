defmodule Core.EntryChannel do
  ...
  defp extract_update(entry, campaign_name) do
    base = %{
      "address" => process_address(entry["address"]),
      "phone" => entry["phone"],
      "deceased" => entry["deceased"],
      "support_level" => entry["supportScore"]
    }

    base =
      if entry["volunteer"] do
  ...
  defp get_existing_person(entry = %{"id_type" => _id_type}),
    do: entry |> extract_matchers() |> Nb.People.match()
